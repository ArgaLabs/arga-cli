...
- `arga previews twins provision` provisions twins without running a browser test. Runs are private by default; use `--public` only when unauthenticated provider API access is required. Use `--scenario-id` or `--scenario-prompt` to seed them, and `--candidate-safe` to expose only provider data-plane APIs on the public candidate host. Candidate-safe runs hide the twin root/UI, twin-hosted documentation and OpenAPI/schema discovery, and control-plane routes; trusted authenticated admin URLs remain available for seeding and verification.
