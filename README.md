...
- `arga previews twins provision` provisions twins without running a browser test. Runs are private by default; use `--candidate-safe` when only the provider data-plane APIs must be reachable unauthenticated, and `--public` when full unauthenticated access (UI, root, docs, OpenAPI) is required. Use `--scenario-id` or `--scenario-prompt` to seed them. Candidate-safe runs hide the twin root/UI, twin-hosted documentation and OpenAPI/schema discovery, and control-plane routes; trusted authenticated admin URLs remain available for seeding and verification.
